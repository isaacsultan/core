...
contract('Broker', function ([_, adminRole, brokerRole]) {
  beforeEach(async function () {
    this.managingDirector = await ManagingDirector.new(toBytes('inverse'), adminRole, brokerRole);
    this.erc20TellerFactory = await Erc20TellerFactory.new(adminRole);
    this.ethTeller = await EthTeller.new(this.managingDirector.address, adminRole);
    ...
    this.broker = await Broker.new(
      ...
      this.basicTokenFactory,
      this.compliance,
      this.ethTeller,
      this.erc20TellerFactory,
      adminRole
    );
  });
  ...
  describe('approveProduct()', function() {
    it('should allow an admin to approve a new product type', async function () {
      const { logs } = await this.broker.approveProduct(toBytes('CTB'), toBytes('BTC'), {from: adminRole});
      expectEvent.inLogs(logs, 'NewProduct', {productType: toBytes('BTC'), underlyingType: toBytes('CTB')});
    });
  });
  describe('agree()', function () {
    it ('should revert if an invalid product type is unapproved', async function() {
      await shouldFail.reverting(this.broker.agree('DENT'));
    });
    context('a valid product type is approved', function() {
      beforeEach(async function() {
        await this.broker.approveProduct(toBytes('CTB'), toBytes('BTC'), {from: adminRole});
      });
  ...
  describe('offerCollateral()', function () {
    it('should require a valid collateral type', async function () {
      await this.broker.approveProduct(toBytes('CTB'), toBytes('BTC'), {from: adminRole});
      const id = await this.broker.agree(toBytes('CTB'));
      await shouldFail.reverting(this.broker.offerCollateral(id, toBytes('DENT'), 1));
    });
    it('should require a valid agreementID', async function () {
      await shouldFail.reverting(this.broker.offerCollateral(0, toBytes('ETH'), 1));
    });
